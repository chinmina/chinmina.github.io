...
{/* Renovate-managed: keep the plugin#vX.Y.Z form. */}
...
## Creating tokens through a different GitHub App

Permissions granted by a profile are bounded by the GitHub App that creates the token. When a profile needs permissions the default app does not hold, register an additional app and name it with `app`:
...
The `pr-commenter` profile omits `app`, so it uses the default app. The `publish` profile creates tokens through the `packages` app instead.

Apps are registered in the deployment configuration, not in this file. See [Using multiple GitHub Apps](/guides/multiple-github-apps).
...
See [Using multiple GitHub Apps](/guides/multiple-github-apps) to register an additional app and select it from a profile.
