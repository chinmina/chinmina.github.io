...
This application becomes Chinmina's **default app**, used by every profile that
does not name another one. Additional apps can be registered later, as described
in [Using multiple GitHub Apps](/guides/multiple-github-apps).
...
- **Either**:
  - <ConfigRef name="GITHUB_APP_PRIVATE_KEY_ARN" />
    ...
    and provide the ARN of the key's **alias** here. This will allow Chinmina to
    sign tokens but protects the key from extraction and misuse.
